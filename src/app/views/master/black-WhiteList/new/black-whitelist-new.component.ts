...
export class BlackWhitelistNewComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.addBlackWhiteForm.patchValue({
      receiveNo: [''],
      sku: [''],
      serialNoNew: [''],
      serialNoOld: [''],
  ...
  onSave() {
    ...
    if (
      this.addBlackWhiteForm.value.recordDate == '' ||
      this.addBlackWhiteForm.value.recordDate == null
    ) {
      Swal.fire({
        title: 'กรุณาตรวจสอบข้อมูล Record Date',
      ...
      if (this.validate.checking(validateData)) {
        ...
        Swal.fire({
          ...
          confirmButtonText:
            '<i class="vp-save-solid"></i><a style="margin-left:10px;margin-right:10px">Save</a>',
          cancelButtonText:
          ...
          if (result.value) {
            ...
            Swal.fire({
              ...
              if (result.value == true) {
                ...
                $("input[name=StatusOptions][value='N']").prop(
                  'checked',
                  false
                );
              }
